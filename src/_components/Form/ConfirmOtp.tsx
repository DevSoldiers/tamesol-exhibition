    ...
    <div className="w-full flex items-center justify-center bg-gradient-to-r from-[#f28f37] to-[#f7a838]">
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-xl">
        ...
        <form onSubmit={formik.handleSubmit}>
          <div className="flex justify-center space-x-4 mb-6">
            {[0, 1, 2, 3].map((index) => (
              <input
                ...
                className={`w-12 h-12 text-center text-2xl border ${Array.isArray(formik.touched.otp) &&
          ...
          {message && (
            <p
              className={`mt-4 text-center ${message.includes('successfully') ? 'text-green-600' : 'text-[#b51f1b]'
                }`}
            >
              {message}
            </p>
