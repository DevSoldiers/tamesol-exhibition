...
export const authService = {
    ...
    async register({
        ...
        try {
            ...
            //     }
            // });
            const user = signIn("Signup", { redirect: false, phoneNumber, password, otp, images })
            return user;
        } catch (err: any) {
            throw Error(
                err?.response?.data?.message
    ...
    async sendOtp({ phoneNumber }: { phoneNumber: string }) {
        try {
            const otp = await api.post(
                `/otp/sendotp?phoneNumber=${phoneNumber}`
            );
    ...
    async sendOtpForForgetPassword({ phoneNumber }: { phoneNumber: string }) {
        try {
            const otp = await api.post(
                `/otp/sendForgotPasswordOtp?phoneNumber=${phoneNumber}`
            );
            return otp.data.data;
        } catch (err: any) {
            throw Error(
